...
mod tests {
    ...
    async fn test_insert_and_verify_task(db: &MongoDataSource) -> ObjectId {
        ...
        let existing_tasks = db
            .find_tasks(Some(doc! { "name": "unit-test-task" }), None)
            .await
            .unwrap();
    ...
    async fn test_insert_and_verify_instance(db: &MongoDataSource) -> ObjectId {
        ...
        let existing_instances = db
            .find_task_instances(Some(doc! { "task_id": task_id }), None)
            .await
            .unwrap();
    ...
    async fn test_insert_and_verify_log(db: &MongoDataSource) -> ObjectId {
        ...
        let existing_logs = db
            .find_execution_logs(Some(doc! { "task_id": task_id }), None)
            .await
            .unwrap();
    ...
    async fn test_task_instance_update() {
        let db = get_test_db().await;
        let instance_id = db.find_task_instances(Some(doc! {}), None).await.unwrap()[0]
            .id
            .unwrap();
        test_update_and_verify_instance(&db, instance_id).await;
    }
    ...
    async fn test_execution_log_update() {
        let db = get_test_db().await;
        let log_id = db.find_execution_logs(Some(doc! {}), None).await.unwrap()[0]
            .id
            .unwrap();
        test_update_and_verify_log(&db, log_id).await;
    }
